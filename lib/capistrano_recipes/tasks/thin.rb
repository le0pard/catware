  ...
  namespace :deploy do
    ...
    namespace :thin do
      ...
      task :setup, :except => { :no_release => true } do
        ...
        run "cd #{current_path}; bundle exec thin config -C #{shared_path}/config/thin.yml -c #{current_path} -e #{fetch(:rails_env, "production")} -d -s 2 -u #{fetch(:user, "deployer")} -g #{fetch(:group, "deployer")} -l #{shared_path}/log/thin.log -P #{shared_path}/tmp/pids/thin.pid -S #{shared_path}/tmp/sockets/thin.sock"
      end
